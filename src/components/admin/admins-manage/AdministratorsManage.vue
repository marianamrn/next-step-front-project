<template>
  <div class="admin-manage-root">
    ...
    <div v-else class="data-table">
      ...
      <div class="table-body" v-if="admins.length > 0">
        <div v-for="admin in admins" :key="admin.id" class="table-row">
          <div class="cell col-name">{{ admin.name }}</div>
          <div class="cell col-surname">{{ admin.last_name }}</div>
    ...
    <!-- Діалог додавання адміністратора -->
    <v-dialog v-model="addDialog" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="addAdmin" :disabled="!selectedUserId">Додати</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  components: { ViewAdminModal, AdminEditModal },
  data() {
    return {
      admins: [],
      users: [],
