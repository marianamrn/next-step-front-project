<template>
  <div class="teacher-detail-wrapper">
    <div class="teacher-detail-breadcrumbs">
      <router-link to="/admin/teachers" class="crumb">Викладачі</router-link>
    ...
    <div class="teacher-detail-card">
      <div class="teacher-detail-main">
        <div class="teacher-detail-left">
          <div class="avatar-big">{{ getInitials(teacher) }}</div>
          <div class="photo-label">Фото викладача</div>
          <div class="reg-date">Зареєстровано: {{ formatDate(teacher?.created_at) }}</div>
        </div>
        <div class="teacher-detail-right">
          ...
          <div class="fields-row">
            ...
            <div class="field-col">
              ...
              <input type="text" :value="teacher?.last_name || ''" disabled />
            </div>
          </div>
          <div class="fields-row">
            <div class="field-col">
            ...
            <div class="field-col phone-group">
              ...
              <div class="phone-fields">
                <input type="text" :value="teacher?.country_code || '+380'" class="phone-code" disabled />
                <input type="text" :value="teacher?.phone_number || ''" class="phone-number" disabled />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="teacher-detail-sections">
      ...
      <div class="teacher-detail-actions">
        <v-btn
          v-if="teacher && teacher.role?.name !== 'super_admin' && teacher.role?.name !== 'admin'"
          color="error"
  ...
  methods: {
    async fetchTeacher() {
      ...
      try {
        ...
        this.teacher = res.data?.user || res.data;
      } finally {
        this.loading = false;
      }
    },
    ...
    async deactivateTeacher() {
      ...
      if (!confirm('Ви впевнені, що хочете деактивувати цього викладача?')) return;
      try {
        await studentsApi.deactivateStudent(this.teacher.id);
        this.$router.push('/admin/teachers');
      } catch (e) {
        alert('Помилка при деактивації викладача');
      }
    },
